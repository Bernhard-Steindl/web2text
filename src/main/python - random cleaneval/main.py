...
def test_structured(lamb=EDGE_LAMBDA):
  ...
  with tf.Session() as session:
    ...
    pd.DataFrame([[accuracy, precision, recall, f1, accuracy_u, precision_u, recall_u, f1_u]], columns=["accuracy", "precision", "recall", "f1", "accuracy_u", "precision_u", "recall_u", "f1_u"]).to_csv(f"cleaneval_decay_random_lambda.csv",mode='a', header=False)
